...
const totalSupplyKey = "100000000"
...
func (s *SmartContract) CreateAccount(ctx contractapi.TransactionContextInterface, did string, emailId string, name string) error {
	exists, err := s.AccountExists(ctx, did)
	if err != nil {
		return err
	}
	if exists {
	...
	return ctx.GetStub().PutState(did, accountJSON)
}

// AssetExists returns true when asset with given ID exists in world state
func (s *SmartContract) AccountExists(ctx contractapi.TransactionContextInterface, did string) (bool, error) {
...
func (s *SmartContract) GetAccountById(ctx contractapi.TransactionContextInterface, DID string) (*Account, error) {
	if len(DID) == 0 {
		return nil, fmt.Errorf("Please provide correct contract Id")
		// return shim.Error("Incorrect number of arguments. Expecting 1")
	}

	accountAsBytes, err := ctx.GetStub().GetState(DID)
...
// This function triggers a Transfer event
func (s *SmartContract) Transfer(ctx contractapi.TransactionContextInterface, minter string, recipient string, amount int) error {
	...
	err := transferHelper(ctx, minter, recipient, amount)
	if err != nil {
		return fmt.Errorf("failed to transfer: %v", err)
	}

	// Emit the Transfer event
	transferEvent := event{minter, recipient, amount}
	transferEventJSON, err := json.Marshal(transferEvent)
	if err != nil {
		return fmt.Errorf("failed to obtain JSON encoding: %v", err)
	}
...
func (s *SmartContract) BalanceOf(ctx contractapi.TransactionContextInterface, minter string) (int, error) {
	balanceBytes, err := ctx.GetStub().GetState(minter)
	if err != nil {
		return 0, fmt.Errorf("failed to read from world state: %v", err)
	}
	if balanceBytes == nil {
		return 0, fmt.Errorf("the account %s does not exist", minter)
	}
...
func (s *SmartContract) Approve(ctx contractapi.TransactionContextInterface, minter string, spender string, value int) error {
	...
	// Create allowanceKey
	allowanceKey, err := ctx.GetStub().CreateCompositeKey(allowancePrefix, []string{minter, spender})
	if err != nil {
	...
	approvalEvent := event{minter, spender, value}
	...
	log.Printf("client %s approved a withdrawal allowance of %d for spender %s", minter, value, spender)

	return nil
}

// Allowance returns the amount still available for the spender to withdraw from the owner
func (s *SmartContract) Allowance(ctx contractapi.TransactionContextInterface, minter string, spender string) (int, error) {
	...
	log.Printf("The allowance left for spender %s to withdraw from owner %s: %d", spender, minter, allowance)
